...
## 6. 앱에서 붙기 — 먼저 같은 LAN 인지 본다

터널이니 포트포워딩이니 하기 전에 **개발 PC 와 DB 서버가 같은 네트워크인지부터** 확인한다.
같다면 아무것도 열 필요가 없다.

```sh
curl -s https://ifconfig.me     # 개발 PC 의 공인 IP
```

이 값이 **서버의 공인 IP 와 같으면 같은 공유기 아래**에 있는 것이다.
그러면 공인 IP 대신 서버의 사설 IP 로 바로 붙는다.

```sh
hostname -I                     # 서버에서
```

실제로 이번이 그 경우였다. 공인 IP(`112.160.125.11`)로 붙을 때는 6001 이 막혔는데,
공유기에 3306 만 포워딩 규칙이 있고 6001 은 없었기 때문이다. 서버의 LAN 주소
(`192.168.0.254`)로 가면 공유기를 거치지 않으므로 그냥 붙는다.
...
**한참 헤맨 뒤에야 알았다.** 터널을 만들고 포트 개방을 검토하기 전에 이걸 먼저 보면
둘 다 필요 없는 경우가 있다.

서버 IP 를 모르면 대역을 훑어도 된다 (내 네트워크 안이므로):

```powershell
1..254 | ForEach-Object { Test-NetConnection -ComputerName "192.168.0.$_" -Port 6001 -InformationLevel Quiet }
```
