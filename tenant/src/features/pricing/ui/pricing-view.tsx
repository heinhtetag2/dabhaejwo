    ...
    <>
      <section className="mx-auto max-w-[1080px] px-5 pt-16 pb-12 text-center sm:pt-24">
        ...
        <p className="mx-auto mt-5 max-w-[520px] text-[16px] leading-[1.75] text-slate sm:text-[17px]">
          14일 무료로 먼저 써보세요. 카드 등록이 필요 없고, 체험 기간에는 요금이 청구되지
          않습니다.
        </p>
      </section>
      ...
      {plans.length === 0 ? (
        ...
        <>
          ...
          <PublicSection tone="fill" eyebrow="한도" title="한도를 넘어도 요금이 늘지 않습니다">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-panel bg-card p-7 sm:p-8">
                ...
                <p className="mt-3 text-[14.5px] leading-[1.75] text-slate">
                  미리 등록해 둔 답변은 AI를 거치지 않고 그대로 나갑니다. 즉시 표시되고 대화
                  수에도 잡히지 않으므로, 자주 오는 질문을 등록해 두시면 한도를 훨씬 덜 씁니다.
                </p>
              </div>
            </div>
          </PublicSection>
          ...
          <PublicSection
            eyebrow="결제"
            title="지금은 담당자가 직접 안내해 드립니다"
            description={
              /* 없는 기능을 있는 것처럼 적지 않는다 (§2.3). 카드 결제는 아직 없다. */
              <>
                <p>
                  대시보드에서 요금제를 신청하시면 1영업일 안에 연락드려 계약과 수납을
                  도와드립니다. 카드 자동 결제는 준비 중입니다.
                </p>
                <p className="mt-4">세금계산서가 필요하시면 신청할 때 함께 알려주세요.</p>
              </>
            }
          />
        </>
...
function PlanCard({ plan }: { plan: PublicPlan }) {
  ...
  return (
    ...
    >
      <h2 className={cn("text-[16px] font-bold tracking-[-0.02em]", free && "text-mark")}>
        {plan.name}
      </h2>

      <p className="mt-4 flex items-baseline gap-1.5">
        {plan.negotiable ? (
          <span className="text-[28px] font-bold tracking-[-0.035em]">협의</span>
        ) : free ? (
          <span className="text-[28px] font-bold tracking-[-0.035em]">무료</span>
        ) : (
          <>
            <span className="tabular text-[30px] font-bold tracking-[-0.04em]">
              {plan.monthlyFee.toLocaleString()}
            </span>
            <span className="text-[14px] text-slate">원 / 월</span>
          </>
        )}
      </p>
      ...
      >
        <Row label="월 대화" value={limitLabel(plan.convLimit, "건")} muted={free} />
        <Row label="학습 문서" value={limitLabel(plan.docLimit, "개")} muted={free} />
      </dl>
        ...
        {plan.negotiable ? (
          ...
          <LinkButton
            ...
            className={cn(
              ...
              free
                ? "border-mark bg-mark text-ink hover:brightness-105"
